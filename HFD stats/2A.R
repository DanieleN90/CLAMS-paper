...
### let's start with TEE (here called Heat, Supplemental FIle 2Ai)
anova(lmer(Heat ~ Group*Sex + Week + (1|Mouse), current_data))

### now let's do locomotor activity (here called LA, Supplemental FIle 2Aii)
anova(lmer(LA ~ Group*Sex + Week + (1|Mouse), current_data))

### now let's do bodyweight (Supplemental File 2Aiii)
anova(lmer(Bodyweight ~ Group*Sex + Week + (1|Mouse), current_data))

### post-hoc analysis for bodyweight, first looking at the contrast between sexes across weeks (2Avi)
...
#and now the contrast between rearing temperatures within each sex, divided per week (2Aix)
# 
...
### now let's do lean mass (2Aiv)
anova(lmer(Lean_Mass ~ Group*Sex + Week + (1|Mouse), current_data))

# post-hoc for lean mass, looking at contrasts between sexes per each week (2Avii)
...
# post-hoc for lean mass, looking at contrasts between sexes per each week (2Aviii)
